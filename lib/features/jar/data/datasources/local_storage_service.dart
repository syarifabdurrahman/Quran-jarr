...
class LocalStorageService {
  ...
  Future<void> saveTodayVerse(VerseModel verse) async {
    await _aBox.put(AppConstants.keyTodayVerse, verse.toJson());
    await _aBox.put('${AppConstants.keyTodayVerse}_timestamp', DateTime.now().toIso8601String());
  }

  /// Get today's verse (only if less than 24 hours old)
  Future<VerseModel?> getTodayVerse() async {
    final json = _aBox.get(AppConstants.keyTodayVerse);
    if (json == null) return null;

    // Check timestamp
    final timestampStr = _aBox.get('${AppConstants.keyTodayVerse}_timestamp');
  ...
  Future<void> saveVerseToArchive(VerseModel verse) async {
    ...
    await _vBox.put(verse.verseKey, json);

    if (!_vBox.containsKey(verse.verseKey)) {
  ...
  Future<bool> isVerseSaved(String verseKey) async {
    return _vBox.containsKey(verseKey);
  }
  ...
  /// Get verse count
  int get savedVerseCount => _vBox.length;
  ...
  Future<void> saveNotificationVerse(VerseModel verse) async {
    await _aBox.put('notification_verse', verse.toJson());
    await _aBox.put('notification_verse_timestamp', DateTime.now().toIso8601String());
  }

  /// Get notification verse (returns null if not found or not from today)
  Future<VerseModel?> getNotificationVerse() async {
    final json = _aBox.get('notification_verse');
    if (json == null) return null;

    // Check timestamp - only return if from today
    final timestampStr = _aBox.get('notification_verse_timestamp');
    if (timestampStr != null) {
      final timestamp = DateTime.parse(timestampStr as String);
      final now = DateTime.now();
  ...
  Future<void> clearNotificationVerse() async {
    await _aBox.delete('notification_verse');
    await _aBox.delete('notification_verse_timestamp');
  }
